...
USBP sectors with jurisdiction in the Pacific Northwest are Blaine Sector ("BLW") and Spokane Sector ("SPW"):
...
OFO field offices in Pacific Northwest include Seattle Field Office and Portland Field Office.
...
p1 <- cbp %>% 
  filter(aor %in% c('Seattle', 'Portland', "SPW", "BLW")) %>% 
  group_by(encounter_type, fiscal_year, month_abb) %>% 
  summarize(n = sum(encounter_count)) %>% 
  ggplot(aes(x = month_abb, y = n, color = fiscal_year, group=fiscal_year)) + 
  geom_line() +
